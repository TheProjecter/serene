...
class ModularValidation{	
	static SchemaFactory schemaFactory;
	
	static DifferentiatedErrorHandler errorHandler;
	...
	static void init(){
		errorHandler = new DifferentiatedErrorHandler();
	...
	public static void main(String[] args){				
		init();
		File sourceDir = new File("data/modularValidation");
	...
	static boolean initReader(File schemaFile){
        ...
        if(errorHandler.hasUnrecoverableError())return false;		
		ValidatorHandler vh = schema.newValidatorHandler();
        String configDescr = "VALIDATOR CONFIGURATION:";
                
        if(errorHandler.hasMissingDatatypeLibraryError()){
            try{
                if(schemaFactory.getFeature(Constants.REPLACE_MISSING_LIBRARY_FEATURE))
                    configDescr += "\nMissing datatype libraries were replaced with native library, type token.";
            }catch (SAXNotRecognizedException e) {
                e.printStackTrace();
            }catch (SAXNotSupportedException e) {
                e.printStackTrace();
        ...
        if(errorHandler.hasAttributeDefaultValueError()){
            configDescr += "\n"+Constants.LEVEL2_ATTRIBUTE_DEFAULT_VALUE_FEATURE +" false";
            try{
                vh.setFeature(Constants.LEVEL2_ATTRIBUTE_DEFAULT_VALUE_FEATURE, false);
            }catch (SAXNotRecognizedException e) {
                e.printStackTrace();
            }catch (SAXNotSupportedException e) {
                e.printStackTrace();
            ...
            configDescr += "\n"+Constants.LEVEL2_ATTRIBUTE_DEFAULT_VALUE_FEATURE +" true";
        }
        
        if(errorHandler.hasAttributeIdTypeError()){
            configDescr += "\n"+Constants.LEVEL1_ATTRIBUTE_ID_TYPE_FEATURE +" false";
            configDescr += "\n"+Constants.LEVEL2_ATTRIBUTE_ID_TYPE_FEATURE +" false";
            try{
                vh.setFeature(Constants.LEVEL1_ATTRIBUTE_ID_TYPE_FEATURE, false);
                vh.setFeature(Constants.LEVEL2_ATTRIBUTE_ID_TYPE_FEATURE, false);
            }catch (SAXNotRecognizedException e) {
                e.printStackTrace();
            }catch (SAXNotSupportedException e) {
                e.printStackTrace();
            ...
            configDescr += "\n"+Constants.LEVEL1_ATTRIBUTE_ID_TYPE_FEATURE +" true";
            configDescr += "\n"+Constants.LEVEL2_ATTRIBUTE_ID_TYPE_FEATURE +" true";
        }        
        System.out.println(configDescr);

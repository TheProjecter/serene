	...
	void handleContentOrder(APattern sourceDefinition){
		...
		for(int i = currentIndex+1; i < size; i++){
			...
			if(minimalStartedCount.get(i) >= maxCount){	
				...
				errorCatcher.misplacedElement(rule, starttSystemId, starttLineNumber, starttColumnNumber, currentChild, validationItemLocator.getQName(), validationItemLocator.getSystemId(), validationItemLocator.getLineNumber(), validationItemLocator.getColumnNumber(), sourceDefinition, reper);

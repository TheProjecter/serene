...
public class GroupHandler extends MultipleChildrenPatternHandler{
    ...
    */	
	...
	public boolean handleContentOrder(int expectedOrderHandlingCount, SPattern child, SPattern sourceDefinition){
	    ...
	    /*System.out.println("GROUP HANDLER    HANDLE CONTENT ORDER  rule="+rule);
	...
	public boolean handleContentOrder(int expectedOrderHandlingCount, SPattern child, SPattern sourceDefinition, InternalConflictResolver resolver){
		...
		if(stackConflictsHandler != null && stackConflictsHandler.isConflictRule(child)){
			stackConflictsHandler.record(child, rule, resolver);
		}
		if(newChildIndex < prevCorrectChildIndex){			
			// add here the reduce/reset part if the contentHandler is satisfied and reduction acceptable			
			// return; the error is neihter located nor reported
	...
	public GroupHandler getCopy(StackHandler stackHandler, ErrorCatcher errorCatcher){
		GroupHandler copy = pool.getCopy(this,
		                                    rule,
		                                    errorCatcher, 
											parent, 
											stackHandler);
	...
	void setEmptyState(){
		if(stackConflictsHandler != null){
			stackConflictsHandler.close(rule);
		}
		super.setEmptyState();
	...
	public String toString(){
		//return "GroupHandler "+hashCode()+" "+rule.toString()+" "+satisfactionLevel+"/"+satisfactionIndicator+" contentHandler "+contentHandler.toString();
		return "GroupHandler  "+rule.toString()+" "+satisfactionLevel+"/"+satisfactionIndicator+" "+saturationLevel+"/"+saturationIndicator+" contentIndex="+contentIndex;
	}

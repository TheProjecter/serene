...
public class MInterleaveHandler extends InterleaveHandler{
	...
	MInterleaveHandler(){
	    ...
	    definition = new SPattern[childInitialSize];
	    
		secondaryHandlers = new ArrayList<SInterleaveHandler>();	
		lastShiftIndex = new IntList();
	}	
	
	void init(SMultipleChildrenPattern interleave, ErrorCatcher errorCatcher, StructureHandler parent, StackHandler stackHandler, ValidatorRuleHandlerPool pool){
	...
	public MInterleaveHandler getCopy(StackHandler stackHandler, ErrorCatcher errorCatcher){
		...
		MInterleaveHandler copy = pool.getCopy(this, rule, errorCatcher, parent, stackHandler);
	...
	void handleParticleShift(int inputRecordIndex, SPattern cd){
	...
	int computeOccurrencesCount(){
		...
		currentSecondaryHandler = pool.getSInterleaveHandler(rule, errorCatcher, null, stackHandler, this);
	...
	void increaseChildrenStorageSize(){
		...
		SPattern[] increasedD = new SPattern[newSize];
